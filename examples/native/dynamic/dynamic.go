...
func DynamicExample() error {
	...
	if err := proton_tests.CheckMinServerVersion(conn, 2, 9); err != nil {
		fmt.Print("unsupported proton version for dynamic type")
		return nil
	}

	err = conn.Exec(ctx, "DROP STREAM IF EXISTS go_dynamic_example")
	if err != nil {
		return err
	}

	err = conn.Exec(ctx, `
		CREATE STREAM go_dynamic_example (
		    c dynamic
		) ENGINE = Memory
	`)
	if err != nil {
	...
	if err = batch.Append(proton.NewDynamic("example dynamic")); err != nil {
		return err
	}

	if err = batch.Append(proton.NewDynamicWithType("example dynamic with specific type", "string")); err != nil {
		return err
	}
