...
func DynamicExample() error {
	...
	if _, err = batch.ExecContext(ctx, proton.NewDynamic("example dynamic")); err != nil {
		return err
	}

	if _, err = batch.ExecContext(ctx, proton.NewDynamicWithType("example dynamic with specific type", "string")); err != nil {
		return err
	}
