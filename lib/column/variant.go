...
func (c *Variant) Row(i int, ptr bool) interface{} {
...
func (c *Variant) ScanRow(dest interface{}, row int) error {
...
func (c *Variant) Append(v interface{}) (nulls []uint8, err error) {
	switch vv := v.(type) {
	case []chcol.Variant:
		for i, vt := range vv {
			err := c.AppendRow(vt)
...
func (c *Variant) AppendRow(v interface{}) error {
...
func (c *Variant) encodeHeader(encoder *binary.Encoder) error {
	return encoder.UInt64(SupportedVariantSerializationVersion)
}

func (c *Variant) encodeData(encoder *binary.Encoder) error {
	if err := encoder.Raw(c.discriminators); err != nil {
		return err
	}
...
func (c *Variant) WriteStatePrefix(encoder *binary.Encoder) error {
	return c.encodeHeader(encoder)
}

func (c *Variant) Encode(encoder *binary.Encoder) error {
...
func (c *Variant) decodeHeader(decoder *binary.Decoder) error {
	variantSerializationVersion, err := decoder.UInt64()
	if err != nil {
...
func (c *Variant) decodeData(decoder *binary.Decoder, rows int) error {
	...
	for i, col := range c.columns {
		...
		if err := col.Decode(decoder, cRows); err != nil {
...
func (c *Variant) ReadStatePrefix(decoder *binary.Decoder) error {
...
func (c *Variant) Decode(decoder *binary.Decoder, rows int) error {
