...
func (c *JSON) scanIntoStruct(dest interface{}, row int) error {
...
func (c *JSON) scanIntoMap(dest interface{}, row int) error {
	val := reflect.ValueOf(dest)
	if val.Kind() != reflect.Ptr {
		return fmt.Errorf("destination must be a pointer")
	}
...
func structToJSON(v interface{}) (*chcol.JSON, error) {
	json := chcol.NewJSON()
...
func mapToJSON(v interface{}) (*chcol.JSON, error) {
	json := chcol.NewJSON()
	val := reflect.ValueOf(v)
