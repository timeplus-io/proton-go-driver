...
func TestStdIPv4(t *testing.T) {
	if conn, err := sql.Open("proton", "proton://127.0.0.1:8463"); assert.NoError(t, err) {
		...
		`
		defer func() {
			conn.Exec("DROP STREAM test_std_ipv4")
		}()
		if _, err := conn.Exec(ddl); assert.NoError(t, err) {
			...
			if batch, err := scope.Prepare("INSERT INTO test_std_ipv4 (* except (_tp_time, _tp_sn))"); assert.NoError(t, err) {
				var (
				...
				if _, err := batch.Exec(col1Data, col2Data, &col3Data, &col4Data, &col5Data); assert.NoError(t, err) {
					if assert.NoError(t, scope.Commit()) {
						...
						if err := conn.QueryRow("SELECT (* except _tp_time) FROM test_std_ipv4 WHERE _tp_time > earliest_ts() LIMIT 1").Scan(&col1, &col2, &col3, &col4, &col5); assert.NoError(t, err) {
