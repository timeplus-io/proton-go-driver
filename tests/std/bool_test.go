...
func TestStdBool(t *testing.T) {
	if conn, err := sql.Open("proton", "proton://127.0.0.1:8463"); assert.NoError(t, err) {
		...
		`
		defer func() {
			conn.Exec("DROP STREAM test_std_bool")
		}()
		if _, err := conn.Exec(ddl); assert.NoError(t, err) {
			...
			if batch, err := scope.Prepare("INSERT INTO test_std_bool (* except (_tp_time, _tp_sn))"); assert.NoError(t, err) {
				var val bool
				if _, err := batch.Exec(true, false, []bool{true, false, true}, nil, []*bool{&val, nil, &val}); assert.NoError(t, err) {
					if err := scope.Commit(); assert.NoError(t, err) {
						...
						if err := conn.QueryRow("SELECT (* except _tp_time) FROM test_std_bool WHERE _tp_time > earliest_ts() LIMIT 1").Scan(&col1, &col2, &col3, &col4, &col5); assert.NoError(t, err) {
