...
func TestIssue389(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		if err := conn.Exec(ctx, ddl); assert.NoError(t, err) {
			if batch, err := conn.PrepareBatch(ctx, "INSERT INTO issue_389 (* except (_tp_time, _tp_sn))"); assert.NoError(t, err) {
				if err := batch.Append(int64(1625128291293)); assert.NoError(t, err) {
					if err := batch.Send(); assert.NoError(t, err) {
						var col1 time.Time

						if err := conn.QueryRow(ctx, "SELECT (* except _tp_time) FROM issue_389 WHERE _tp_time > earliest_ts() LIMIT 1").Scan(&col1); assert.NoError(t, err) {
